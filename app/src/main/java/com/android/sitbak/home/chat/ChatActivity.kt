        ...
        viewModel.getSocketReceiveMessage().observe(this, {
            if (it != null) {
                if (it.order_id == orderId) {
                    chatList.add(
                        ChatData(
                            it.chat_id, it.created_at, it.file_path, it.id, it.order_id, it.is_delivered, it.is_seen,
    ...
    private fun checkForIntent(intent: Intent?) {
        ...
        if (intent != null) {
            ...
            if (intent.hasExtra("senderName"))
                binding.tvChatTitle.text = intent.getStringExtra("senderName")
